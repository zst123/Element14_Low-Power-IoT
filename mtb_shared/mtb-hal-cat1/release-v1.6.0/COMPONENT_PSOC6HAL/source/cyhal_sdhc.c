...
typedef struct _cyhal_sdxx_stored_obj
{
    void * obj;
    bool   is_sdio;
} _cyhal_sdxx_stored_obj_t;

static _cyhal_sdxx_stored_obj_t stored_objects[CY_IP_MXSDHC_INSTANCES];
...
static cy_rslt_t _cyhal_sdxx_prepare_for_transfer(SDHC_Type *base)
{
    CY_ASSERT(base != NULL);
    uint8_t block_num = (base == SDHC0) ? 0 : 1;
    CY_ASSERT(stored_objects[block_num].obj != NULL);

    bool is_sdio = stored_objects[block_num].is_sdio;

    uint32_t activated_cmd_complete = 0;
    if (((is_sdio) && (((cyhal_sdio_t *)stored_objects[block_num].obj)->irq_cause & CYHAL_SDIO_CMD_COMPLETE)) ||
        ((!is_sdio) && (((cyhal_sdhc_t *)stored_objects[block_num].obj)->irq_cause & CYHAL_SDHC_CMD_COMPLETE)))
    {
...
static bool _cyhal_sdxx_handle_weak_func(const SDHC_Type *base, _cyhal_sdhc_weak_func_type weak_function, bool enable)
{
    ...
    CY_ASSERT(NULL != stored_objects[block_idx].obj);
    ...
    bool is_sdio = stored_objects[block_idx].is_sdio;

    if (is_sdio)
    {
        pin_pwr_en = NC;
        pin_io_volt_sel = ((cyhal_sdio_t *)stored_objects[block_idx].obj)->pin_io_vol_sel;
        pin_card_detect = NC;
        pin_write_prot = NC;
    }
    else
    {
        cyhal_sdhc_t *obj = (cyhal_sdhc_t *)stored_objects[block_idx].obj;
        pin_pwr_en = obj->pin_card_pwr_en;
        pin_io_volt_sel = obj->pin_io_vol_sel;
        pin_card_detect = obj->pin_card_detect;
        pin_write_prot = obj->pin_card_mech_write_prot;
    }
    ...
    *   GPIO used for io voltage switching. */
    if (_CYHAL_SDHC_CARD_IO_VOLTAGE == weak_function)
    {
        ...
        if (is_sdio)
        {
            ((cyhal_sdio_t *)stored_objects[block_idx].obj)->low_voltage_io_set = enable;
        }
        else
        {
            ((cyhal_sdhc_t *)stored_objects[block_idx].obj)->low_voltage_io_set = enable;
        }
    }
...
*
*******************************************************************************/
static cy_en_sd_host_status_t _cyhal_sdxx_polltransfercomplete(SDHC_Type *base, const uint16_t delay)
{
    cy_en_sd_host_status_t ret = CY_SD_HOST_ERROR_TIMEOUT;
    ...
    while ((CY_SD_HOST_ERROR_TIMEOUT == ret) && (retry-- > 0U))
    {
        ...
        if (CY_SD_HOST_XFER_COMPLETE == (CY_SD_HOST_XFER_COMPLETE & status) ||
            ((is_sdio) && (!cyhal_sdio_is_busy((cyhal_sdio_t *)stored_objects[block_idx].obj))) ||
            ((!is_sdio) && (!cyhal_sdhc_is_busy((cyhal_sdhc_t *)stored_objects[block_idx].obj))))
        {
            /* Transfer complete */
            ret = CY_SD_HOST_SUCCESS;
            break;
...
static cy_rslt_t _cyhal_sdxx_waitfor_transfer_complete(uint32_t block_num, SDHC_Type *base)
{
    ...
    #endif
        CY_UNUSED_PARAMETER(block_num);
        ret = (cy_rslt_t)_cyhal_sdxx_polltransfercomplete(base, _CYHAL_SDHC_RW_TIMEOUT_US);
    #if defined(CY_RTOS_AWARE) || defined(COMPONENT_RTOS_AWARE)
    }
    #endif
                        ...
                        cyhal_gpio_t emmc_reset)
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        stored_objects[obj->resource.block_num].obj = (void *)obj;
        stored_objects[obj->resource.block_num].is_sdio = false;
...
void cyhal_sdhc_free(cyhal_sdhc_t *obj)
{
    ...
    if (NULL != obj->base)
    {
        ...
        stored_objects[obj->resource.block_num].obj = NULL;
        _cyhal_sdhc_data_transfer_status[obj->resource.block_num] = _CYHAL_SDHC_NOT_RUNNING;
...
cy_rslt_t cyhal_sdhc_read(const cyhal_sdhc_t *obj, uint32_t address, uint8_t *data, size_t *length)
{
    ...
    if (CY_RSLT_SUCCESS == ret)
    {
        ret = _cyhal_sdxx_waitfor_transfer_complete(obj->resource.block_num, obj->base);
    }
    if(CY_RSLT_SUCCESS != ret)
    {
        _cyhal_sdhc_data_transfer_status[obj->resource.block_num] = _CYHAL_SDHC_NOT_RUNNING;
...
cy_rslt_t cyhal_sdhc_read_async(const cyhal_sdhc_t *obj, uint32_t address, uint8_t *data, size_t *length)
{
    ...
    ret = _cyhal_sdxx_prepare_for_transfer(obj->base);
...
cy_rslt_t cyhal_sdhc_send_cmd(cyhal_sdhc_t *obj, cyhal_sdhc_cmd_config_t *cmd_config)
{
    ...
    if (cmd_data_present)
    {
        result = _cyhal_sdxx_prepare_for_transfer(obj->base);
    }
...
cy_rslt_t cyhal_sdhc_wait_transfer_complete(cyhal_sdhc_t *obj)
{
    CY_ASSERT(NULL != obj);
    CY_ASSERT(NULL != obj->base);
    return _cyhal_sdxx_waitfor_transfer_complete(obj->resource.block_num, obj->base);
}
...
static cy_rslt_t _cyhal_sdhc_io_volt_negotiate(cyhal_sdhc_t *obj, cyhal_sdhc_io_voltage_t io_voltage)
{
    ...
    *  SD bus to 1 bit bus width and 400 khz on the SD card side. Preparing host side for that. */
    ...
    {
        _cyhal_sdhc_wait_ncc_time_at_400khz();
        result = _cyhal_sdxx_waitfor_transfer_complete(obj->resource.block_num, obj->base);
    }
...
static cy_rslt_t cyhal_sdxx_set_io_voltage(void *obj, bool is_sdio, cyhal_sdxx_io_voltage_t io_voltage,
                                           _cyhal_sdxx_io_switch_action_t io_switch_type)
{
    ...
    if ((!is_sdio && (CY_SD_HOST_SD != ((cyhal_sdhc_t *)obj)->context.cardType)) &&
            (_CYHAL_SDXX_IO_VOLT_ACTION_NONE != io_switch_type) && (_CYHAL_SDXX_IO_VOLTAGE_1_8V == io_voltage))
    {
        /* For SDHC, Negotiation or Voltage switch is not supported when eMMC used */
        return CYHAL_SDHC_RSLT_ERR_UNSUPPORTED;
    }
    if (is_sdio && (_CYHAL_SDXX_IO_VOLT_ACTION_NEGOTIATE == io_switch_type))
    {
    ...
    switch (io_switch_type)
    {
        ...
        {
            cyhal_sdhc_t * sdhc_obj = (cyhal_sdhc_t *)obj;
            ...
            *   Saving the parameters to restore them after I/O switch is done. */
            ...
            if ((sdhc_obj->low_voltage_io_set) && (_CYHAL_SDXX_IO_VOLTAGE_3_3V == io_voltage))
            {
        ...
        case _CYHAL_SDXX_IO_VOLT_ACTION_SWITCH_SEQ_ONLY:
            ...
            {
                result = _cyhal_sdxx_io_volt_switch_seq(base);
                if (CY_RSLT_SUCCESS != result)
                {
                    result = is_sdio ? CYHAL_SDHC_RSLT_ERR_IO_VOLT_SWITCH_SEQ : CYHAL_SDIO_RSLT_ERR_IO_VOLT_SWITCH_SEQ;
                }
                break;
            }
        ...
        case _CYHAL_SDXX_IO_VOLT_ACTION_NONE:
            Cy_SD_Host_ChangeIoVoltage(base, (cy_en_sd_host_io_voltage_t)io_voltage);
            break;
...
cy_rslt_t cyhal_sdhc_set_io_voltage(cyhal_sdhc_t *obj, cyhal_sdhc_io_voltage_t io_voltage, cyhal_sdhc_io_volt_action_type_t io_switch_type)
{
    ...
    return cyhal_sdxx_set_io_voltage((void *)obj, false, (cyhal_sdxx_io_voltage_t)io_voltage,
            (_cyhal_sdxx_io_switch_action_t)io_switch_type);
}
        ...
        cyhal_gpio_t data2, cyhal_gpio_t data3)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            obj->base = _CYHAL_SDHC_BASE_ADDRESSES[obj->resource.block_num];
            stored_objects[obj->resource.block_num].obj = (void *)obj;
            stored_objects[obj->resource.block_num].is_sdio = true;
            _cyhal_sdhc_data_transfer_status[obj->resource.block_num] = _CYHAL_SDHC_NOT_RUNNING;
            ...
            if (result == CY_RSLT_SUCCESS)
            {
                ...
                if (result == CY_RSLT_SUCCESS)
                {
                    ...
                    result = _cyhal_sdxx_sdcardchangeclock(obj, true, &freq, obj->low_voltage_io_set, false);
                    if (result == CY_RSLT_SUCCESS)
                    {
...
void cyhal_sdio_enable_event(cyhal_sdio_t *obj, cyhal_sdio_irq_event_t event, uint8_t intr_priority, bool enable)
{
    ...
    {
        uint32_t cur_interrupt_mask = Cy_SD_Host_GetNormalInterruptMask(obj->base);
        ...
        if (enable)
        {
            ...
            *   regardless it was enabled by user or not. */
            ...
            Cy_SD_Host_ClearNormalInterruptStatus(obj->base, new_interrupt_mask & ~cur_interrupt_mask);
        }
        else
        {
            new_interrupt_mask &= ~(event);
        ...
        Cy_SD_Host_SetNormalInterruptMask(obj->base, new_interrupt_mask);
    }
                                    ...
                                    cyhal_sdio_io_volt_action_type_t io_switch_type)
{
    ...
    if (NC != io_volt_sel)
    {
        ...
        {
            result = _cyhal_sdxx_setup_pin(obj, io_volt_sel, cyhal_pin_map_sdhc_io_volt_sel,
                _CYHAL_SDHC_ELEM_COUNT(cyhal_pin_map_sdhc_io_volt_sel), &(obj->pin_io_vol_sel), _CYHAL_SDHC_CARD_IO_VOLTAGE);
        }
        /* io volt sel pin is already configured and user provided other pin */
        else if (io_volt_sel != obj->pin_io_vol_sel)
        {
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        result = cyhal_sdxx_set_io_voltage((void *)obj, true, (cyhal_sdxx_io_voltage_t)io_voltage,
            (_cyhal_sdxx_io_switch_action_t)io_switch_type);
    }
